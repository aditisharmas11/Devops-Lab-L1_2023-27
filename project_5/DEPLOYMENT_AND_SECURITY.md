...
This document outlines the deployment steps and security best practices implemented in SiddhantOps Console by Ayush Siddhant (PRN 23070122066).
...
**Build Image:**
```bash
docker build -t siddhantops-console:latest .
```

**Run via Compose:**
The `docker-compose.yml` file is configured to set up a private bridge network (`ayush-retail-network`). Only the web application exposes port `8081` to the host. The MySQL database port `3306` is isolated within the Docker network.
...
# Scan Docker image
trivy image --severity HIGH,CRITICAL siddhantops-console:latest
```

**Docker Scout:**
Docker Scout analyzes image layers for CVEs (Common Vulnerabilities and Exposures).
```bash
docker scout cves siddhantops-console:latest
```
